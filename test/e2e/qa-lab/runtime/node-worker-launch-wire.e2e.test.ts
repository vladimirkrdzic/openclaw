...
function requireWorkerAdmissionReceipt(result: AuditRunInspectResult) {
  const receipt = result.decisions.find(
    (candidate) => candidate.decision.reasonCode === "worker_admission_gate_allowed",
  );
  expect(receipt).toMatchObject({
    action: { family: "worker", operation: "admit" },
    decision: { outcome: "allowed" },
    enforcement: { coverageState: "enforced" },
    source: { owner: "worker-runtime" },
  });
  return receipt!;
}
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        requireWorkerAdmissionReceipt(JSON.parse(workerAuditAfterRestart) as AuditRunInspectResult);
