...
- Separate repository authorization from GitHub merge enforcement. `CODEOWNERS` routes review requests; a pending request or zero submitted reviews does not prove that approval is mandatory. Restricted/security paths require listed-owner authorship, review, or direction. For governance changes to ownership/review policy itself, explicit direction from a live-verified repository or organization owner also satisfies repository policy; ordinary collaborator `admin` permission does not. Neither route waives a live GitHub-enforced review rule.
- Before reporting a mandatory approval blocker, inspect live branch protection and every matching ruleset, the PR review decision/requests, and the authenticated actor's permission and bypass state. Name the exact enforced rule and whether it is satisfied. Bypass state is evidence about the likely server outcome, never authorization. If no review rule is enforced, do not stop before native prepare/merge solely because a requested team has not reviewed.
- Explicit user direction resolves this repository-policy question only from the applicable listed owner or through the verified repository/organization-owner governance route; it cannot override server enforcement. If GitHub requires an independent approval and it remains unsatisfied, stop with the exact blocker even when the actor has bypass permission. Otherwise continue through the native landing flow and let its verified merge command exercise the live rule.
