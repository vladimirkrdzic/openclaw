...
- `CODEOWNERS` routes reviewers; it does not itself enforce approval. Maint/refactor/tests need no separate owner ask. Larger behavior/product/security/ownership needs listed-owner involvement; owner-authored work or explicit owner direction satisfies this repo policy. Before calling approval mandatory, verify live branch protection/rulesets and PR review state. Never bypass a GitHub-enforced review rule.
