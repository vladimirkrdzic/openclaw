...
- `CODEOWNERS` routes reviewers; it does not itself enforce approval. Maint/refactor/tests need no separate owner ask unless a path has explicit restricted/security ownership; those paths always need listed-owner involvement. Larger behavior/product/security/ownership also needs listed-owner involvement. Owner-authored work or explicit owner direction satisfies this repo policy. Before calling approval mandatory, verify live branch protection/rulesets and PR review state. Never bypass a GitHub-enforced review rule.
