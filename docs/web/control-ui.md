...
Select a receipt to see the requested action family and operation, its safe summary, the recorded outcome and reason code, enforcement classification, durable record owner and decision boundary, evidence limits, and the producer-supplied next step. **Enforced** means the recorded owner changed the outcome after validating the exact context, execution, and run tuple. **Attribution only** records what happened without claiming authorization. **Unsupported** means that observation has no Phase 0 enforcement contract. The inspector displays these states as text badges as well as color and never infers a reason from another field.

Receipt requests are limited to 50 records. **Load more receipts** follows the Gateway's opaque cursor and keeps earlier pages visible. A later-page error does not discard receipts already shown. Each receipt link adds `receipt=<receipt-id>` and, for a later page, `decision=<opaque-cursor>` to the selected run or execution URL. Reloading that link requests the same bounded page and selects the receipt without exposing its identifier as page text. An expired or invalid page cursor is an explicit inspection error; return to the run or execution URL without the receipt parameters to restart from the first page.
...
The audit ledger is best-effort operational evidence, not a lossless compliance archive. A missing or expired record does not prove that a run or action did not occur. Loading, empty, not-found, expired, corrupt, disconnected, unauthorized, unsupported, and pagination failures remain distinct states instead of being collapsed into an inferred result.

The inspector is read-only: it cannot edit, approve, retry, or repeat the recorded action. It never displays prompt or message text, command bodies, arguments, file paths, payloads, credentials, environment values, receipt/action/source identifiers, policy or grant identifiers, or arbitrary plugin data. Counts may show that policy or grant references participated, but their values stay hidden. See [Audit history](/gateway/audit) for collection, privacy, retention, and CLI inspection details.
